                        ...
                        if search_result.get('meta', {}).get('skip_llm', False) or \
                           (search_results and search_results[0].get('file_type') == 'Troubleshooting'):
                            ...
                            if file_type == 'Troubleshooting':
                                ...
                                # 🔥 關鍵修正：從資料庫取得該文件的所有 chunks（不依賴向量搜尋結果）
                                chunks = doc.get('chunks', [])
                                
                                # 如果 chunks 不完整（少於 6 個），從資料庫重新取得所有 chunks
                                    ...
                                    try:
                                        ...
                                        for chunk in db_chunks:
                                            chunks.append({
                                                ...
                                                'similarity': 1.0  # 因為是直接取得，設為滿分
                                            })
                                    except Exception as e:
                                        logger.error(f"  ✗ 從資料庫取得 chunks 失敗: {e}")
                                
                                # 顯示 chunks 內容 (按 8D 結構)
                                if chunks and len(chunks) > 0:
                                    logger.info(f"  ✓ 找到 {len(chunks)} 個 chunks")
                                    # 定義顯示順序
                                    field_order = [
                                    ...
                                    logger.info(f"  - chunk titles: {list(chunk_map.keys())}")
                                    
                                    # 除錯: 顯示每個 chunk 的詳細資訊
                                    for i, chunk in enumerate(chunks):
                                        logger.info(f"  - Chunk {i}: title='{chunk.get('title')}', content_length={len(chunk.get('content', ''))}, keys={list(chunk.keys())}")
                                    ...
                                    # 沒有 chunks，嘗試從資料庫直接讀取
                                    logger.warning(f"  ✗ 無 chunks，嘗試從資料庫讀取")
                                    st.warning("⚠️ 此 8D 報告無內容片段，嘗試從資料庫讀取...")
                                    
                                    # 方法 1: 使用 raw_content
                                    content = doc.get('raw_content', '')
                                    
                                    # 方法 2: 如果 raw_content 也是空的，從 doc_id 讀取所有 chunks
                                    if not content and doc.get('doc_id'):
                                        try:
                                            from core.database.vector_ops import get_chunks_by_doc_id
                                            db_chunks = get_chunks_by_doc_id(doc['doc_id'])
                                            logger.info(f"  - 從 DB 讀取到 {len(db_chunks)} 個 chunks")
                                            
                                            if db_chunks:
                                                for chunk in db_chunks:
                                                    title = chunk.get('source_title', '未命名段落')
                                            ...
                                            logger.error(f"  ✗ 從資料庫讀取失敗: {e}")
                                            st.error(f"❌ 讀取失敗: {e}")
                                    
                                    # 方法 3: 如果還是空的，顯示 raw_content
                                    if content:
                                        st.markdown("### 📄 文件內容")
                                        st.markdown(content)
